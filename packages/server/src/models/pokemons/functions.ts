...
import {Connection, Edge, PageInfo, Pokemon} from '@frontend-assignment/shared';
...
export function filterByType({types = []}: Partial<Filters>) {
    return (p: Pokemon) =>
        types.length ? p.types.some((type) => types.includes(type)) : true;
}

export function filterByName({q = ''}: Partial<Filters>) {
    return (p: Pokemon) => p.name.toLowerCase().startsWith(q.toLowerCase());
}
